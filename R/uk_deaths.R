...
#########################################################
  ...
  { mylm <- lm(F$Deaths ~ F$Week);
    ...
    # the adjustment: NOT:   F$secular <- F$Deaths + slp*(1+WTOT-F$Week); 
    # BUT:
  ...
  # sec <- lm(ukdead$Deaths ~ ukdead$Date); 
  # abline(reg=sec); 
  #uintercept <- sec$coefficients[1]
  #uslope <- sec$coefficients[2]  # the slope in deaths/week based on UNIX time!! 

  MYDEAD <- secular_fix(ukdead); 

  ## adjsec <- lm(MYDEAD$secular ~ MYDEAD$Date); 
  ## plot(x=MYDEAD$Date, y=MYDEAD$secular);
  ## lines(x=MYDEAD$Date, y=MYDEAD$secular, col='red'); 
  ## abline(reg=adjsec, col='red');
