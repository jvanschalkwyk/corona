...
#' 
#' We effectively build a normal distribution or a log-normal one from simple components.  
#' 
...
if( abs(xscale-1) > 0.01 )
  ...
  myplot <- ggplot( DF, aes(x=DAT) ) + geom_histogram(breaks=brk) + 
   ...
   annotation_custom( INTABLE[,2:3], # xmin=0.3, xmax=topyear/3.5, 
                      ymin=0.3*TopGraph, ymax=0.9*TopGraph,
                      xmax=HiLim, xmin=HiLim*0.8 ); 
if( (method == 'add') | log )
  { myplot <- myplot + 
