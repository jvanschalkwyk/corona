...
#'     Depends on gganimate, reshape2  
#' 
#' @param filename=foo.gif writes to this file name (NULL for current GIF device)
...
corona_life <- function (filename='default.gif', side=50, steps=100, density=0.3)
{ ARENA <- matrix(nrow=side, ncol=side);
  ARENA[] <- rbinom(side^2,1,density);
  STORE <- melt( game_of_life(ARENA, steps) ); 
...
# - <Section 32> - 
game_of_life <- function(ARENA, steps=50)
